...
public class CreateCategoryCommand : IRequest<Result<Category, CategoryException>>
{
    ...
    : IRequestHandler<CreateCategoryCommand, Result<Category, CategoryException>>
{
    ...
    {
        var exisitingCategory = await categoryRepository.GetByName(request.Name, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var entity = Category.New(CategoryId.New(), categoryName);
            
            return await categoryRepository.Create(entity, cancellationToken);
        }
        catch (Exception e)
        {
